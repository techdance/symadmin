...
class EasyAdminTwigExtension extends AbstractExtension
{
    ...
    public function __construct(ContainerInterface $container, RolePermissionManager $rolePermissionManager,  ConfigManager $configManager, PropertyAccessorInterface $propertyAccessor, EasyAdminRouter $easyAdminRouter = null, ?LogoutUrlGenerator $logoutUrlGenerator, $translator = null, AuthorizationChecker $authorizationChecker)
    {
    ...
    public function getActionsForItem($view, $entityName)
    {
        ...
        $actions = array_filter($viewActions, function ($action) use ($excludedActions, $disabledActions) {
            return !\in_array($action['name'], $excludedActions) && !\in_array($action['name'], $disabledActions);
        });

        return $this->rolePermissionManager->initRolePermissions()->spliceEasyAdminActions($actions);
    }
    ...
    {
        return $this->rolePermissionManager->initRolePermissions()->getRolePermissions();
    }
